# Loop records — `agent/loops/`: one file per feature recording the passes through the loop

`agent/loops/` holds **loop records** — one file per feature documenting the
...
`agent/loops/` in the format above, recording each pass's hinge → options →
